...
import { ShieldAlert, Check, X, RefreshCw, Users, Shield, Plus, Minus, ArrowUpRight } from 'lucide-react';
...
export default function AdminHub() {
  ...
  const resolveWithdrawal = async (id: string, telegramId: number, amount: number, isApproved: boolean) => {
    setLoading(true);
    if (isApproved) {
      // Balance is already deducted on the user side when they request it. So approving just updates status to APPROVED.
      await supabase.from('withdrawals').update({ status: 'APPROVED', resolved_at: new Date().toISOString() }).eq('id', id);
    } else {
      // Rejecting a withdrawal means refunding the user.
  ...
  return (
    <div className="min-h-[100dvh] bg-obsidian text-white p-4 font-sans no-scrollbar overflow-y-auto pb-20">
             ...
             {pendingWithdrawals.map(wd => (
                <div key={wd.id} className="bg-black/50 border border-white/10 p-3 rounded-xl flex flex-col gap-2">
                   <div className="flex justify-between items-start">
                      <div>
                         <p className="font-mono font-bold text-sm text-white">[{wd.telegram_id}]</p>
                         <p className="font-mono font-bold text-lg text-blue-400 mt-1">₹{wd.amount}</p>
                         <p className="font-mono text-[10px] text-metallic mt-1">Pay To: <span className="text-white bg-white/10 px-1 py-0.5 rounded">{wd.upi_id}</span></p>
                      </div>
                      <div className="flex gap-2">
       ...
       <div className="glass-panel p-4">
          <h2 className="text-[10px] font-bold tracking-[0.2em] text-metallic uppercase mb-3 flex items-center gap-1">
             <Users size={12} /> User Directory ({users.length})
          </h2>
          <div className="space-y-2">
             {users.map(u => (
                <div key={u.telegram_id} className="bg-white/5 border border-white/10 p-3 rounded-xl">
                   <div className="flex justify-between items-center">
                      <div>
                         <p className="font-mono text-xs font-bold text-white flex items-center gap-1">
                           {u.first_name} <span className="text-[9px] text-metallic bg-black/50 px-1 rounded">ID:{u.telegram_id}</span>
                         </p>
                         <p className="font-mono text-sm text-emerald mt-1">{u.balance} CC</p>
                      </div>
                      <button 
                         onClick={() => setAdjustTarget(adjustTarget === u.telegram_id ? null : u.telegram_id)}
